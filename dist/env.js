...
function probeCaseSensitivity(dir) {
    const probe = path.join(dir, ("_shk_case_" + process.pid));
    try {
        writeFileSync(probe, "");
...
function probeUnicodeNormalization(dir) {
    const nfcPath = path.join(dir, ("_shk_u00e9_" + process.pid));
    const nfdPath = path.join(dir, ("_shk_eu0301_" + process.pid));
    try {
...
function probeSymlinks(dir) {
    const target = path.join(dir, ("_shk_symtgt_" + process.pid));
    const link = path.join(dir, ("_shk_symlink_" + process.pid));
    try {
...
export function probeEnvironment(projectDir) {
    const fsProbes = ({
...
export function validateEnvironment(stored, current) {
    const fatal = [];
